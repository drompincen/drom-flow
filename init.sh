...
collect_managed_files() {
  ...
  # a relocated repo-intel graph lives outside .claude/.state and would otherwise be orphaned
  if [ -f "$target/.claude/.state/drom-flow.conf" ]; then
    _ris="$(sed -n 's/^REPO_INTEL_STATE=//p' "$target/.claude/.state/drom-flow.conf" | tail -1)"
    if [ -n "$_ris" ]; then
      case "$_ris" in /*) _risp="$_ris" ;; *) _risp="$target/$_ris" ;; esac
      [ -d "$_risp" ] && managed+=("$_ris")
    fi
  fi
  true
}
...
if [ "$MODE" = "uninstall" ]; then
  ...
  for rel in "${managed[@]}"; do
    # a relocated state directory is recorded as an absolute path and must not be re-rooted
    case "$rel" in /*) target="$rel" ;; *) target="$TARGET_DIR/$rel" ;; esac
    if [ -d "$target" ]; then
      rm -rf "$target"
      echo "  remove: $rel"
      removed=$((removed + 1))
  ...
  echo ""
  # The engine directory is entirely ours: remove it whole rather than file-by-file, so an
  # older release's files cannot survive by not being in today's template.
  if [ -d "$TARGET_DIR/.claude/df" ]; then
    rm -rf "$TARGET_DIR/.claude/df"
    echo "  remove: .claude/df/ (repo-intel runtime)"
  fi
  dir_removed=0
  for d in "${MANAGED_DIRS[@]}"; do
    target="$TARGET_DIR/$d"
...
# Merge, never truncate: this file also carries host settings such as REPO_INTEL_STATE, and
# rewriting it wholesale on every update silently forgets them.
_conf="$TARGET_DIR/.claude/.state/drom-flow.conf"
if [ -f "$_conf" ]; then
  grep -v '^DROM_FLOW_HOME=' "$_conf" > "$_conf.tmp" 2>/dev/null || : > "$_conf.tmp"
  echo "DROM_FLOW_HOME=$SCRIPT_DIR" >> "$_conf.tmp"
  mv -f "$_conf.tmp" "$_conf"
else
  echo "DROM_FLOW_HOME=$SCRIPT_DIR" > "$_conf"
fi
...
# The repo-intel launcher has no .sh suffix, so the globs above miss it. Without +x the
# capability silently never runs.
chmod +x "$TARGET_DIR/.claude/df/repo-intel/run" 2>/dev/null || true
mkdir -p "$TARGET_DIR/.claude/.state/repo-intel" 2>/dev/null || true
...
echo "  .claude/df/repo-intel/ — automatic repository structure awareness (no setup required)"
