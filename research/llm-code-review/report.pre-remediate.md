...
- **Security:** hybrid SAST/LLM limits and high incomplete-case static FP [S12]; vendor/auth catch anecdote (catch, not residual) [S24]; marketing security framing [S15]—no stratified LLM-vs-human security recall on matched PRs.
...
- **Severity bands (vendor self-bench):** mid-to-high catch by band on Greptile’s own set [S18]; unreplicated.
...
- **Languages/frameworks:** no cross-language effect sizes for LLM-vs-human defect catch in corpus (product marketing may name multi-language stacks, e.g. Java/Python static/LLM hybrid work [S12], but that is not stratified review effectiveness).
...
- **Tuning / filtering / prompt/tooling:** Defaults vs months of custom instructions and noise filters materially change “effectiveness” [S21][S23].
